...
class Wappalyzer:
    def __init__(self, target, output_filepath):
        ...
        self.wappalyzer_image_name = 'wappalyzer/cli'
        ...
        try:
            self.client.images.get(self.wappalyzer_image_name)
        except ImageNotFound:
            print(f'[-] Image {self.wappalyzer_image_name} not exist. pull it first.')
            print(f'[*] Waiting to pull or pull it manually by "docker pull {self.wappalyzer_image_name}".')
            self.client.images.pull(self.wappalyzer_image_name)
    ...
    def scan(self):
        print(f'[*] Running wappalyzer container.')
        container = self.client.containers.run(self.wappalyzer_image_name, self.target, auto_remove=True)
...
def run():
    ...
    parser = argparse.ArgumentParser(description="Wappalyzer")
    parser.add_argument(
        '-t', '--target', dest='target', action='store', required=True,
        help='target that wappalyzer scan technologies on it.'
    )
    parser.add_argument(
        '-o', '--output', dest='output_filepath', action='store', default='wappalyzer.json',
        help='file to save results. default:"wappalyzer.json"'
    )

    args = parser.parse_args()
    wappalyzer = Wappalyzer(**vars(args))
    wappalyzer.scan()
    print('[+] Done.')
